...
int main(void)
{
    ...
    P1REN |= BIT1;  //enable pull-up resistor on 1.1
    P1OUT |= BIT1;  //enable pull-up resistor on 1.1
    ...
    P1IFG &= ~BIT1;  //clear interrupt flags
                                               ...
                                               //Timer1
    TA1CTL = TASSEL__SMCLK | MC__CONTINUOUS;   // timer A0
    TA1CCR0 = 64;                              //Sets Timer reset 0
    TA1CCTL0 |= CCIE;                          //enable interrupt

                                      //Timer0
    TA0CTL = TASSEL__SMCLK | MC__UP; // timer A0
...
__interrupt void TIMER1_A0 (void)
{
    ...
    if(!(P1IN & BIT1) && (P1IES & BIT1)) //if button is pressed
    {
        ...
        if(TA0CCR1 <= 900) //if the duty cycle is not at 100%
        {
